...
class CloudTranslationWriterSystemTest extends KernelTestCase
{
     ...
     * @requires extension blackfire
     */
    public function testWithCacheWriteAndApcuUsage()
    {
        $configuration = new Configuration();
        $configuration->assert('main.peak_memory < 70kB', 'CloudTranslationWriter apcu usage memory usage');
        $configuration->assert('main.network_in < 30B', 'CloudTranslationWriter apcu usage network in');
        $configuration->assert('main.network_out < 1MB', 'CloudTranslationWriter apcu usage network out');
